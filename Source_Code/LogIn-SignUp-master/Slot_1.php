...
$slotStateQry = "SELECT `state` FROM `slot_1` WHERE `no` = 1";
...
$bookingUp = "UPDATE `booking_up` SET `name` = '$u_name', `number_plate` = '$n_plate', `slot_no` = '1' WHERE `booking_up`.`SR_NO` = 1";
mysqli_query($conn, $bookingUp);

$slotUpdate = "UPDATE `slot_1` SET `state` = 'booked' WHERE `slot_1`.`no` = 1";
mysqli_query($conn, $slotUpdate);

$bookLog = "INSERT INTO `booking` (`name`, `number_plate`, `slot_no`) VALUES ('$u_name', '$n_plate', '1')";
